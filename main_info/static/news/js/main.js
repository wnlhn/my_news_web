$(function(){
    ...
    $(".register_form_con").submit(function (e) {
		...
		if (!mobile) {
            $("#register-mobile-err").html('手机号不能为空!')
            $("#register-mobile-err").show();
            return;
        }
        if (!smscode) {
            $("#register-sms-code-err").show('请填写图片验证码!');
...
function generateImageCode() {
    ...
    //拼接请求路径
    image_url = '/passport?cur_id=' + imageCodeId + '&pre_id=' + preId
    // 将image_url替换到默认图片的src中
    ...
    $.ajax({
        ...
        success:function(resp){
            ...
            if (resp.errno == '0'){
                ...
                $('.get_code').attr('onclick','sendSMSCode()') //重新设置可以点击
            }
        }
    })
}
